import numpy as np
import sys
from src import assign1_Q2_main as pre
  ...
  # Calculate Motion Vector
    for bl_y_it in range(n_y_blocks) :
      for bl_x_it in range(n_x_blocks):
        ...
        string_mv = "Frame: %d, bl_y: %d, bl_x: %d, [%d, %d]\n" % (frame, bl_y_it, bl_x_it, mv[frame][bl_y_it][bl_x_it][0], mv[frame][bl_y_it][bl_x_it][1])
        mv_txt.write(string_mv)
